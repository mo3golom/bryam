...
describe('TempoScrollEngine', () => {
  ...
  it('start sets isActive and isPaused correctly', () => {
    ...
    const engine: any = new TempoScrollEngine(song, mockStateChange, 120, 4)

    expect(engine._state.isActive).toBe(false)
    expect(engine._state.isPaused).toBe(false)

    engine.start()

    expect(engine._state.isActive).toBe(true)
    expect(engine._state.isPaused).toBe(false)
  })
  ...
  it('advances activeLineIndex and activeChordIndex over time', () => {
    ...
    advance(engine, 0)
    expect(engine._state.activeLineIndex).toBe(0)
    expect(engine._state.activeChordIndex).toBe(0)

    // after 3000ms: still in line 0 (lineDuration 4000ms), chordIndex should be 1
    advance(engine, 3000)
    expect(engine._state.activeLineIndex).toBe(0)
    expect(engine._state.activeChordIndex).toBe(1)

    // after additional 2000ms -> total 5000ms: should be in line 1 (second line)
    advance(engine, 2000)
    expect(engine._state.activeLineIndex).toBe(1)
    expect(engine._state.activeChordIndex).toBe(0)
  ...
  it('setBpm recalculates durations and keeps progress consistent', () => {
    ...
    advance(engine, 100)
    expect(engine._state.currentBpm).toBe(240)
  })
})
